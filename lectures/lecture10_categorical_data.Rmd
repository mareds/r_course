...
manta_rays$month <- factor(
  manta_rays$month, levels = 1:12,
  labels = c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec")
)
p1 <- ggplot(manta_rays, aes(x = month, y = catch)) +
  geom_col() + labs(x = "Months", y = "Catch")
p2 <- ggplot(manta_rays, aes(y = catch,
  x = forcats::fct_reorder(month, catch))) +
  geom_col() + labs(x = "Sorted Months", y = "Catch")
